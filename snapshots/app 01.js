...
// Function to initiate ngrok and set the Twilio webhook
async function initiateNgrokTwilio() {
  try {
    // Start ngrok and set the forwarding URL
    const url = await ngrok.connect(8080); // Replace 8080 with your server port
    ...
    const webhookUrl = `${url}/twiml`; // The TwiML URL for handling calls or messages
    await client.incomingPhoneNumbers
      .list({ phoneNumber: twilioPhoneNumber })
      .then(incomingPhoneNumbers => {
        incomingPhoneNumbers.forEach(number => {
          client.incomingPhoneNumbers(number.sid).update({ smsUrl: webhookUrl, voiceUrl: webhookUrl });
          console.log(`Twilio webhook URL updated for ${twilioPhoneNumber}: ${webhookUrl}`);
        });
      });
  } catch (error) {
...
const mediaws = new WebSocketServer({
  httpServer: wsserver,
  autoAcceptConnections: false, // Change to false to manually accept connections
});

// Array to keep track of connected clients
const clientConnections = [];

// Override console.log to broadcast messages to connected clients
...
const WebSocketClient = require('ws'); // Rename to WebSocketClient to avoid confusion
...
<Response>
  <Say>Speak now.</Say>
  <Connect>
    <Stream url="wss://${streamUrl}/streams">
...
mediaws.on('request', function (request) {
  ...
  if (request.resourceURL.path === '/logs') {
    // Accept connection from client for logs
    ...
    connection.on('close', function (reasonCode, description) {
      ...
      var index = clientConnections.indexOf(connection);
      if (index !== -1) {
        clientConnections.splice(index, 1);
      }
    });
  } else {
    // Assume it's a Twilio connection
    var connection = request.accept(null, request.origin);
